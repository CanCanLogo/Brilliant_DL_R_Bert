...
from Exp3_DataSet import TextDataSet_new as TextDataSet
from Exp3_Model import Bert_changed
...
def train(num_epochs, batch_size, data_loader_train, data_loader_valid):
    ...
    for epoch in range(num_epochs):
        ...
        for data in tqdm_iterator:
            ...
            # 模型预测
            # y_hat = model(batch_x)
            mask = batch_x['attention_mask'].squeeze(1).to(device)
            input_id = batch_x['input_ids'].squeeze(1).to(device)
            e1_mask = batch_x['e1_mask'].squeeze(1).to(device)
            e2_mask = batch_x['e2_mask'].squeeze(1).to(device)
            # 通过模型得到输出
            output = model(input_id, mask, e1_mask, e2_mask)
            # print(y_hat)
        ...
        if valid_acc > max_valid_acc:
            ...
            torch.save(model, "model_bert_changed.pth")
...
def predict(data_loader_test):
    ...
    # 写入文件
    with open("exp3_predict_labels_1120211392_new.txt", "w") as f:
...
if __name__ == '__main__':
    ...
    model = Bert_changed(ntoken=config.max_token_per_sent,
                         d_emb=config.embedding_dimension,
                         num_classes=config.label_num).to(device)
    # torch.save(model, "model_bert_changed.pth")
    # model = torch.load("model_bert_changed.pth").to(device)
